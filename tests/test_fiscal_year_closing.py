...
async def test_fiscal_year_closing_flow(container):
    ...
    async with AsyncExitStack() as stack:
        ...
        fy_service = await stack.enter_async_context(
            container.fiscal_year_service_scope()
        )
        ...
        await journal_service.add_journal_entry(
            Transaction(
                date=fy_start,
        ...
        txs = await journal_service.get_entries(
        ...
        # Cash (12000 Dr)
        l_cash = next(
            (tx_line for tx_line in lines if tx_line.account_id == acc_cash.id), None
        )
        ...
        # Capital (5000 Cr)
        l_cap = next(
            (tx_line for tx_line in lines if tx_line.account_id == acc_capital.id), None
        )
        ...
        l_re = next(
            (tx_line for tx_line in lines if tx_line.account_id == acc_re.id), None
        )
