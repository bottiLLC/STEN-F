...
async def test_frequent_accounts_manual_wiring(test_session):
    ...
    async def run_test(mock_di):
        async def add_via_service(account_ids, is_deleted=False):
            ...
            lines = [
                TransactionLine(account_id=aid, debit=100, credit=0)
            ...
            lines.append(
                TransactionLine(account_id=id_cash, debit=0, credit=total_debit)
            )

            tx = Transaction(date=date.today(), description="Test", lines=lines)
