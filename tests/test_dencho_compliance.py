...
async def test_dencho_compliance_features(container):
    ...
    async with AsyncExitStack() as stack:
        ...
        # Update same CP
        cp_update = Counterparty(
            name="Test Corp Updated", invoice_number="T1234567890123"
        )
        ...
        class MockFileService:
            async def save_evidence_for_transaction(
                self, file_bytes, transaction_id, date_obj, amount, corp_name
            ):
        ...
        tx_id = await journal_service.add_journal_entry_with_evidence(
            tx, file_bytes, mock_file_service
        )
