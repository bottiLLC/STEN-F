    ...
    async def handle_upload(self, files: List[rx.UploadFile]):
        ...
        try:
            for file in files:
                ...
                # Call OCR
                receipt_data = await ocr_service.extract_receipt_data(
                    upload_data, file_type, acc_options, cp_options
                )
                ...
                if receipt_data:
                    ...
                    if receipt_data.is_registered_merchant:
                        yield rx.toast(
                            f"登録済みの取引先「{receipt_data.merchant_name}」と一致しました。",
                            duration=5000,
                            close_button=True,
                        )
                    else:
                        yield rx.toast(
                            "AI読み取り完了（新規取引先の可能性があります）",
                            duration=5000,
                            close_button=True,
                        )
                else:
        ...
        if data.total_amount_incl_tax:
            ...
            if not debit_acc and form_state.counterparty:
                async with DI.get_master_service() as service:
                    cps = await service.get_counterparties()
                    matched = next(
                        (c for c in cps if c.name == form_state.counterparty), None
                    )
                    if matched:
            ...
            form_state.lines = [
                {
                    ...
                    "debit": data.total_amount_incl_tax,
                    "credit": 0,
                },
                {
                    "account_id": credit_acc,
                    "debit": 0,
                    "credit": data.total_amount_incl_tax,
                },
            ]
