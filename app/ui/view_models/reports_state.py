...
class ReportsState(rx.State):
    ...
    def set_report_date(self, v: str):
        self.report_date = v

    def set_audit_date(self, v: str):
        self.audit_date = v
    ...
    async def load_fiscal_years(self):
        ...
        async with DI.get_master_service() as service:
            ...
            # Build options
            self.fy_options = [
                f"{fy.name} (第{fy.period_number}期)" for fy in self.fiscal_years
            ]
            self.fy_map = {
                f"{fy.name} (第{fy.period_number}期)": str(fy.id)
    ...
    async def load_report_data(self):
        ...
        try:
            ...
            async with DI.get_ledger_service() as service:
                ...
                self.financial_report = await service.generate_financial_report(
                    fy_id_int
                )
