...
st.caption("事業者情報、会計年度・決算締め処理、期首残高、勘定科目・取引先・摘要マスタ、バックアップ、AI設定を一括管理します。")

tab_corp, tab_fy, tab_op, tab_acc, tab_cp, tab_abs, tab_backup, tab_sys = st.tabs([
    "🏢 自社情報", "📅 会計年度・年度締め", "⚖️ 期首残高設定",
    "📑 勘定科目", "🤝 取引先", "💬 よく使う摘要", "💾 バックアップ", "⚙️ AI・システム設定"
])
...
with tab_corp:
    ...
    with st.form("corp_form"):
        ...
        corp_addr = st.text_input("本店所在地 / 住所", value=corp.address if corp and corp.address else "")
        c1, c2 = st.columns(2)
        corp_rep_title = c1.text_input("代表者役職", value=corp.representative_title if corp and corp.representative_title else "代表社員")
        corp_rep_name = c2.text_input("代表者氏名", value=corp.representative_name if corp and corp.representative_name else "")

        if st.form_submit_button("💾 自社情報を保存する", type="primary", use_container_width=True):
            if not corp_name.strip():
                ...
                updated_corp = Corporation(
                    ...
                    representative_title=corp_rep_title.strip() if corp_rep_title else None,
                    representative_name=corp_rep_name.strip() if corp_rep_name else None,
                )
    ...
    with col_fy_list:
            ...
            fy_rows = [
                {
                    ...
                    "開始日": str(f.start_date), "終了日": str(f.end_date),
                    "状態": "進行中 (OPEN)" if f.status == "OPEN" else "締切済 (CLOSED)",
                }
                for f in sorted(fys, key=lambda x: x.start_date, reverse=True)
            ]
            st.dataframe(pd.DataFrame(fy_rows), hide_index=True, use_container_width=True)
        else:
        ...
        if open_fy:
            ...
            with st.form("fy_close_form"):
                next_fy_name = st.text_input("次期年度名称", value=f"第{(open_fy.period_number or 0) + 1}期")
                if st.form_submit_button("⚠️ この会計年度を締め切る (CLOSED)", type="primary", use_container_width=True):
        ...
        with st.form("new_fy_form"):
            ...
            if st.form_submit_button("登録する", type="primary", use_container_width=True):
                if new_start >= new_end:
                    ...
                    new_fy = FiscalYear(name=new_name.strip(), period_number=int(new_period), start_date=new_start, end_date=new_end, status="OPEN")
...
with tab_op:
    ...
    async def fetch_op_data():
        async with DI.get_master_service() as s:
            ...
            return next((x for x in f if x.status == "OPEN"), None), await s.get_accounts()

    open_fy, all_accounts = run_async(fetch_op_data())
    if not open_fy:
        st.warning("進行中 (OPEN) の会計年度がありません。")
    else:
        debit_accs = [a for a in all_accounts if a.type in (AccountType.CURRENT_ASSET, AccountType.FIXED_ASSET, AccountType.DEFERRED_ASSET)]
        credit_accs = [a for a in all_accounts if a.type in (AccountType.CURRENT_LIABILITY, AccountType.FIXED_LIABILITY, AccountType.EQUITY)]
        ...
        with st.form("opening_balance_form"):
            ...
            total_d, total_c = sum(int(v) for v in op_d.values()), sum(int(v) for v in op_c.values())
            st.markdown("---")
...
with tab_acc:
    ...
    acc_df = pd.DataFrame([
        {"id": a.id, "code": a.code, "name": a.name, "type": a.type.value if hasattr(a.type, "value") else str(a.type), "description": a.description or ""}
            ...
            async with DI.get_master_service() as s:
                for r in added:
                    if r.get("code") and r.get("name") and r.get("type"):
                        await s.save_account(Account(code=str(r["code"]), name=str(r["name"]), type=AccountType(r["type"]), description=r.get("description")))
                for pk, chg in edited.items():
                    cur = next((a for a in acc_list if a.id == pk), None)
                    if cur:
                        await s.save_account(Account(id=pk, code=str(chg.get("code", cur.code)), name=str(chg.get("name", cur.name)), type=AccountType(chg.get("type", cur.type)), description=chg.get("description", cur.description)))
                for pk in deleted:
    ...
    col_cfg_acc = {
        ...
        "type": st.column_config.SelectboxColumn("勘定区分", options=[t.value for t in AccountType], required=True),
        "description": st.column_config.TextColumn("説明・用途"),
    }
    render_accounting_editor(acc_df, pk_column="id", base_key="editor_accounts", on_commit=on_commit_accounts, column_config=col_cfg_acc)
...
with tab_cp:
    ...
    cp_df = pd.DataFrame([{"id": c.id, "name": c.name, "invoice_number": c.invoice_number or "", "debit_account_id": c.debit_account_id or None, "credit_account_id": c.credit_account_id or None} for c in cps])
    ...
    def on_commit_cps(added, edited, deleted):
        async def do_commit():
            async with DI.get_master_service() as s:
                for r in added:
                    if r.get("name"):
                        await s.save_counterparty(Counterparty(name=str(r["name"]), invoice_number=r.get("invoice_number") or None, debit_account_id=int(r["debit_account_id"]) if r.get("debit_account_id") else None, credit_account_id=int(r["credit_account_id"]) if r.get("credit_account_id") else None))
                for pk, chg in edited.items():
                    cur = next((c for c in cps if c.id == pk), None)
                    if cur:
                        await s.save_counterparty(Counterparty(id=pk, name=str(chg.get("name", cur.name)), invoice_number=chg.get("invoice_number", cur.invoice_number), debit_account_id=int(chg["debit_account_id"]) if chg.get("debit_account_id") else cur.debit_account_id, credit_account_id=int(chg["credit_account_id"]) if chg.get("credit_account_id") else cur.credit_account_id))
                for pk in deleted:
        ...
        async def do_commit():
            async with DI.get_master_service() as s:
                ...
                for pk, chg in edited.items():
                    ...
                    if cur:
                        await s.save_abstract(Abstract(id=pk, text=str(chg.get("text", cur.text)), account_id=int(chg.get("account_id", cur.account_id))))
                for pk in deleted:
...
with tab_sys:
    ...
    with st.form("sys_form"):
        api_key_input = st.text_input("Gemini API キー", value=sys_conf.ai_api_key or "", type="password", help="Google AI Studio で発行された API キー")
        if st.form_submit_button("設定を保存する", type="primary", use_container_width=True):
            async def save_sys(k):
                async with DI.get_master_service() as s:
                    await s.save_system_settings(SystemSettings(ai_api_key=k if k else None))
