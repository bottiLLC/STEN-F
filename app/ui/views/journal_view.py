...
st.caption("AI OCR（領収書・請求書自動読取）起点での振替伝票作成、および仕訳帳の一覧・検索・CSV出力を一元的に行います。")
...
account_labels = [""] + [f"{a.code}: {a.name}" for a in sorted(accounts, key=lambda x: int(x.code))]
...
with tab_entry:
    ...
    uploaded_file = st.file_uploader("領収書・請求書をドラッグ＆ドロップ", type=["pdf", "png", "jpg", "jpeg"], key="journal_file_uploader")

    if uploaded_file and st.button("🤖 AIで自動読み取りを実行", type="primary", key="btn_run_ocr"):
    ...
    with col_h2:
        abs_choice = st.selectbox("よく使う摘要から選ぶ", abstract_options, key="tx_header_abs_choice")
        tx_desc = st.text_input(
            "伝票摘要 (取引内容)",
            value=abs_choice if abs_choice else (ocr.description if ocr and ocr.description else ""),
            key="tx_header_desc",
        )
    with col_h3:
        tx_cp = st.text_input("取引先", value=ocr.merchant_name if ocr and ocr.merchant_name else "", key="tx_header_cp")
    with col_h4:
        tx_inv = st.text_input("インボイス番号 (T+13桁)", value=ocr.invoice_registration_number if ocr and ocr.invoice_registration_number else "", key="tx_header_inv")
    ...
    lines_df = pd.DataFrame([{
        "debit_account": d_init, "debit_amount": init_amt,
        "credit_account": c_init, "credit_amount": init_amt,
        "line_desc": ocr.description if ocr and ocr.description else "",
    }])
    col_cfg = {
        "debit_account": st.column_config.SelectboxColumn("借方科目", options=account_labels),
        "debit_amount": st.column_config.NumberColumn("借方金額 (¥)", min_value=0, step=1000, required=True),
        "credit_account": st.column_config.SelectboxColumn("貸方科目", options=account_labels),
        "credit_amount": st.column_config.NumberColumn("貸方金額 (¥)", min_value=0, step=1000, required=True),
        "line_desc": st.column_config.TextColumn("行摘要"),
    }

    st.markdown("##### 【 振替伝票 明細行 】")
    edited_lines_df = st.data_editor(lines_df, column_config=col_cfg, num_rows="dynamic", use_container_width=True, hide_index=True, key="journal_voucher_lines_editor")
    ...
    col_m3.metric("貸借バランス", "✅ 一致" if is_balanced else f"差額: ¥{total_debit - total_credit:,}")

    if st.button("💾 この内容で仕訳帳に登録する", type="primary", disabled=not is_balanced, use_container_width=True):
        new_tx = Transaction(
            date=tx_date,
        ...
        async def commit_tx(tx_obj: Transaction, fb: Optional[bytes] = None):
            async with DI.get_journal_service() as s:
                if fb:
                    return await s.add_journal_entry_with_evidence(tx_obj, fb, DI.get_file_service())
...
with tab_history:
    ...
    s_date = st.date_input("開始日", value=open_fy.start_date if open_fy else date(date.today().year, 1, 1), key="hist_s_date")
    e_date = st.date_input("終了日", value=open_fy.end_date if open_fy else date(date.today().year, 12, 31), key="hist_e_date")
    ...
    if not entries:
        ...
        for tx in entries:
            ...
            for i in range(max(len(d_lines), len(c_lines), 1)):
                ...
                rows.append({
                    "id": tx.id,
                    "取引日": str(tx.date),
                    "借方科目": account_id_to_label.get(d.account_id, "") if d else "",
                    "借方金額": d.debit if d else 0,
                    "貸方科目": account_id_to_label.get(c.account_id, "") if c else "",
        ...
        st.download_button("📥 仕訳帳 CSV をエクスポート", data=csv_buf.getvalue().encode("utf_8_sig"), file_name=f"journal_{s_date}_{e_date}.csv", mime="text/csv")
