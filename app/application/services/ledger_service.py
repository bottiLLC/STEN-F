...
class LedgerService:
    def __init__(self, repository: ILedgerRepository):
        ...
        self.log = logger.bind(service="LedgerService")
    ...
    async def get_general_ledger(self, fiscal_year_id: int, account_id: int) -> pd.DataFrame:
        """
        ...
        try:
            ...
            transactions = await self.repository.get_transactions_by_account(
                account_id, 
                start_date=target_fy.start_date, 
                end_date=target_fy.end_date
            )
    ...
    async def _generate_financial_report_logic(self, fiscal_year_id: int):
        from domain.models.financial_report import FinancialReport, FinancialSection, FiscalYear
