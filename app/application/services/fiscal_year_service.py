...
class FiscalYearService:
    ...
    async def close_fiscal_year(
        ...
        exp_types = {AccountType.COST_OF_SALES, AccountType.SGA, AccountType.NON_OPERATING_EXPENSE, AccountType.EXTRAORDINARY_LOSS, AccountType.TAXES}
        inc_types = {AccountType.REVENUE, AccountType.NON_OPERATING_INCOME, AccountType.EXTRAORDINARY_INCOME}
        ...
        re_row = next((r for r in tb_rows if r.account_name == "繰越利益剰余金"), None) or next((r for r in tb_rows if r.account_code == "3120"), None)
        if not re_row:
        ...
        total_d, total_c = sum(line.debit for line in lines), sum(line.credit for line in lines)
        if total_d != total_c:
            raise ValueError(f"Opening Balance unbalanced: Dr {total_d} != Cr {total_c}")
