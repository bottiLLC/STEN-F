        ...
        try:
            ...
            next_fy = next(
                (
                    fy
                    for fy in all_fys
                    if fy.period_number == (current_fy.period_number or 0) + 1
                ),
                None,
            )
            ...
            if not next_fy:
                ...
                determined_name = (
                    next_fy_name
                    if next_fy_name
                    else f"第{(current_fy.period_number or 0) + 1}期"
                )
            ...
            if not retained_earnings_row:
                retained_earnings_row = next(
                    (r for r in tb_rows if r.account_code == "3120"), None
                )
            ...
            opening_tx = Transaction(
                date=next_fy.start_date, description="前期繰越", lines=lines
            )
            ...
            if total_debit != total_credit:
                raise ValueError(
                    f"Opening Balance unbalanced: Dr {total_debit} != Cr {total_credit}"
                )
