...
class PDFService:
    ...
    def generate_annual_report(
        ...
        y_left = draw_bs_section(
            rpt.current_assets, "流動資産合計", rpt.current_assets.total, y_left
        )
        y_left = draw_bs_section(
            rpt.fixed_assets, "固定資産合計", rpt.fixed_assets.total, y_left
        )
        if rpt.deferred_assets.total > 0:
            y_left = draw_bs_section(
                rpt.deferred_assets, "繰延資産合計", rpt.deferred_assets.total, y_left
            )
        ...
        y_right = draw_bs_section(
            rpt.fixed_liabilities, "固定負債合計", rpt.fixed_liabilities.total, y_right
        )
        ...
        c.drawRightString(
            180 * mm, y_right, f"{rpt.total_liabilities + rpt.total_equity:,}"
        )
        ...
        def draw_pl_section_obj(section):
            ...
            c.drawString(
                x_label + 5 * mm,
                y,
                f"{section.title.replace('【', '').replace('】', '')} 合計",
            )
        ...
        def draw_detail_table(section):
            ...
            c.setFont(settings.FONT_NAME, 11)
            c.drawString(
                30 * mm, y, f"{section.title.replace('【', '').replace('】', '')} 明細"
            )
            y -= 6 * mm
        ...
        if corp.representative_title and corp.representative_name:
            c.drawString(
                30 * mm, y, f"{corp.representative_title}  {corp.representative_name}"
            )
