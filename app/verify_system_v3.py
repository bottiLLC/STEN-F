...
async def verify_system():
    ...
    if target_fy:
        ...
        print("❌ No Fiscal Year available for testing.")
        return
    ...
    try:
        ...
        if tx_id is not None:
            if not gl_df.empty:
                # Simple ID check if column exists
                if 'TransactionID' in gl_df.columns:
                    ...
                    if not target_row.empty:
                     ...
                     print("❌ GL DataFrame missing TransactionID column!")
            else:
                 print(f"❌ General Ledger is empty for {cash_acc.name}!")
    except Exception as e:
    ...
    # Separate blocks to avoid nesting issues
    if tx_id:
        try:
