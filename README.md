...
*   **無償・プロプライエタリ制約ゼロ**: 目的を問わず、無料で自由に使用および改変が可能です。
*   **【重要】コピーレフトの義務**: あなたが本ソフトウェア（あるいは一部を改変した派生物）を再配布する場合、またはネットワークサービスとして外部に機能提供する場合、**そのソースコード全体を同様にGPL-3.0ライセンスの下で一般公開する義務**が生じます。

詳細や完全なライセンス条項については、リポジトリ内の `LICENSE` ファイルを参照してください。

---

## ⚠️ 免責事項・税務リスクに関する強い警告 (Disclaimer & Tax Compliance)

> [!CAUTION]
> **本システムを利用する前に、必ず以下の免責事項をお読みください。本システムの利用を開始した時点で、利用者は以下の事項にすべて同意したものとみなされます。**
> 
> * **完全な現状有姿（As-Is）**: 本システムは「現状有姿」で提供され、動作保証や特定の目的への適合性について、いかなる保証もありません。
> * **利用者による全責任**: 本システムには電子帳簿保存法等の法令への準拠を支援する機能が含まれていますが、最終的な税務申告の適法性や正確性については、**利用者が全責任を負うものとします**。
> * **一切の免責**: システムの不具合やバグによるデータの消失、税務調査での指摘、追徴課税、およびそれに伴う一切の損害について、**開発者（合同会社ぼっち）は一切の責任（損害賠償義務）を負いません**。
> * **自己責任での運用**: 利用にあたっては、定期的なバックアップの取得と、必要に応じて税理士等の専門家への確認を**【自己責任】**で行うこと。
